...
class EnhancedTextViewer:
    def __init__(self, root):
        ...
        self.root.geometry("1280x720")  # 初始尺寸调整为720p
        self.root.minsize(1280, 720)   # 设置最小尺寸为720p
        ...
        self.main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)  # 减少内边距
        ...
        self.right_content.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=(5, 0))  # 减少右边距
    ...
    def on_window_resize(self, event):
        ...
        if event.widget == self.root:
            # 更新PDF显示
            if self.current_pdf and self.pdf_frame.winfo_ismapped():
                self.show_pdf_page()
    ...
    def create_header(self):
        ...
        title_frame.pack(fill=tk.X, pady=(0, 10))  # 减少垂直间距
        
        self.title_label = tk.Label(title_frame, text="分类查看器",
                                  bg="#f0f2f5", fg="#2c3e50", 
                                  font=("Segoe UI", 14, "bold"))  # 减小字体
        self.title_label.pack(side=tk.LEFT)
        
        # 工具栏 - 使用Frame容器并添加滚动条
        toolbar_container = tk.Frame(self.right_content, bg="#f0f2f5")
        toolbar_container.pack(fill=tk.X, pady=(0, 10))
        
        # 添加水平滚动条
        toolbar_scroll_x = tk.Scrollbar(toolbar_container, orient=tk.HORIZONTAL)
        toolbar_scroll_x.pack(side=tk.BOTTOM, fill=tk.X)
        
        # 使用Canvas实现可滚动的工具栏
        self.toolbar_canvas = tk.Canvas(toolbar_container, bg="#f0f2f5", 
                                      height=40,  # 固定高度
                                      xscrollcommand=toolbar_scroll_x.set,
                                      highlightthickness=0)
        self.toolbar_canvas.pack(side=tk.TOP, fill=tk.X)
        
        toolbar_scroll_x.config(command=self.toolbar_canvas.xview)
        
        # 在Canvas上创建工具栏框架
        self.toolbar_frame = tk.Frame(self.toolbar_canvas, bg="#f0f2f5")
        self.toolbar_canvas.create_window((0, 0), window=self.toolbar_frame, anchor="nw")
        
        # 更新工具栏框架大小
        self.toolbar_frame.bind("<Configure>", lambda e: self.toolbar_canvas.configure(
        ...
        tool_btn_style = {"bg": "#3498db", "fg": "white", "bd": 0, 
                         "padx": 8, "pady": 4, "relief": "flat",  # 减少内边距
                         "font": ("Segoe UI", 10)}  # 减小字体
        
        # 导入按钮
        import_btn = tk.Button(self.toolbar_frame, text="📥 导入", command=self.import_file, **tool_btn_style)
        import_btn.pack(side=tk.LEFT, padx=3)
        
        # 保存按钮
        save_btn = tk.Button(self.toolbar_frame, text="💾 保存", command=self.save_file,
                            bg="#2ecc71", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        save_btn.pack(side=tk.LEFT, padx=3)
        
        # 新建按钮
        new_btn = tk.Button(self.toolbar_frame, text="📄 新建", command=self.new_file,
                           bg="#9b59b6", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        new_btn.pack(side=tk.LEFT, padx=3)
        
        # 计算器按钮
        calc_btn = tk.Button(self.toolbar_frame, text="🧮 计算器", command=self.open_calculator,
                            bg="#e67e22", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        calc_btn.pack(side=tk.LEFT, padx=3)
        
        # 修改按钮
        modify_btn = tk.Button(self.toolbar_frame, text="✏ 编辑", command=self.modify_content,
                              bg="#f39c12", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        modify_btn.pack(side=tk.LEFT, padx=3)
        
        # 主页按钮
        home_btn = tk.Button(self.toolbar_frame, text="🏠 主页", command=self.show_home,
                            bg="#34495e", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        home_btn.pack(side=tk.LEFT, padx=3)
        
        # 笔记按钮
        note_btn = tk.Button(self.toolbar_frame, text="📝 笔记", command=self.take_note,
                            bg="#8e44ad", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        note_btn.pack(side=tk.LEFT, padx=3)
        
        # 运行代码按钮
        run_btn = tk.Button(self.toolbar_frame, text="▶️ 运行", command=self.run_python_code,
                            bg="#27ae60", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        run_btn.pack(side=tk.LEFT, padx=3)
        
        # 输出面板控制按钮
        self.output_toggle_btn = tk.Button(self.toolbar_frame, text="📤 输出", command=self.toggle_output_panel,
                            bg="#16a085", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 10))
        self.output_toggle_btn.pack(side=tk.LEFT, padx=3)
        
        # PDF导航按钮
        self.pdf_nav_frame = tk.Frame(self.toolbar_frame, bg="#f0f2f5")
        self.pdf_nav_frame.pack(side=tk.RIGHT, padx=5)
        ...
        time_label = tk.Label(self.toolbar_frame, textvariable=self.time_var, bg="#f0f2f5", 
                             font=("Segoe UI", 9), fg="#666")  # 减小字体
        time_label.pack(side=tk.RIGHT, padx=10)
        
        # 搜索区域
        search_frame = tk.Frame(self.toolbar_frame, bg="#f0f2f5")
        search_frame.pack(side=tk.RIGHT, padx=10)
        
        tk.Label(search_frame, text="🔍 搜索:", bg="#f0f2f5", font=("Segoe UI", 9)).pack(side=tk.LEFT)  # 减小字体
        self.search_entry = tk.Entry(search_frame, width=20, font=("Segoe UI", 9),  # 减小宽度和字体
        ...
        search_btn = tk.Button(search_frame, text="查找", command=self.search_text, 
                              bg="#9b59b6", fg="white", bd=0, padx=8, pady=4, font=("Segoe UI", 9))  # 减小字体
    ...
    def create_global_output_panel(self):
        """创建全局输出面板"""
        # 输出面板框架
        self.output_panel_frame = tk.Frame(self.main_frame, bg="#f0f2f5", height=150)  # 减小高度
        self.output_panel_frame.pack(fill=tk.BOTH, expand=False, pady=(5, 0))  # 减少垂直间距
        
        # 输出面板标题
        output_title_frame = tk.Frame(self.output_panel_frame, bg="#2c3e50", height=25)  # 减小高度
        output_title_frame.pack(fill=tk.X)
        
        tk.Label(output_title_frame, text="输出面板", bg="#2c3e50", fg="white", 
                font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT, padx=10)  # 减小字体
        ...
        tk.Label(output_title_frame, textvariable=self.run_status_var, bg="#2c3e50", fg="#27ae60",
                font=("Segoe UI", 9)).pack(side=tk.RIGHT, padx=10)  # 减小字体
        
        # 控制按钮
        clear_btn = tk.Button(output_title_frame, text="🗑️ 清空", command=self.clear_output,
                             bg="#e74c3c", fg="white", bd=0, padx=6, pady=2, font=("Segoe UI", 9))  # 减小字体
        ...
        self.output_text = scrolledtext.ScrolledText(
            ...
            padx=10, pady=8, highlightthickness=0, border=0,  # 减少内边距
            state="normal", bg="#f8f9fa", fg="#333", font=("Consolas", 10)  # 减小字体
        )
        ...
        self.output_panel_frame.config(height=150)
        
        # 添加分割条
        self.output_sash = tk.Frame(self.main_frame, bg="#bdc3c7", height=3, cursor="sb_v_double_arrow")  # 减小高度
    ...
    def toggle_output_panel(self):
        ...
        if self.output_panel_visible:
            ...
            self.output_panel_frame.pack(fill=tk.BOTH, expand=False, pady=(5, 0))
            self.output_sash.pack(fill=tk.X, pady=0)
    ...
    def resize_output(self, event):
        ...
        new_height = max(80, min(400, self.resize_data["height_start"] - delta_y))  # 限制最小和最大高度
    ...
    def create_navigation_panel(self):
        """创建左侧导航面板"""
        nav_frame = tk.Frame(self.content_frame, bg="#2c3e50", width=220)  # 减小宽度
        nav_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 5))  # 减少右边距
        
        # 导航标题
        nav_title = tk.Label(nav_frame, text="📚 分类导航", bg="#2c3e50", fg="white",
                           font=("Segoe UI", 12, "bold"), pady=10)  # 减小字体
        nav_title.pack(fill=tk.X)
        
        # 导航按钮容器
        nav_buttons_frame = tk.Frame(nav_frame, bg="#2c3e50")
        nav_buttons_frame.pack(fill=tk.X, padx=5, pady=2)  # 减少内边距
        
        # 主页导航按钮
        self.home_nav_btn = tk.Button(nav_buttons_frame, text="🏠 主 页",
                                command=self.show_home,
                                bg="#3498db", fg="white", bd=0, padx=8, pady=6,  # 减少内边距
                                font=("Segoe UI", 10), width=12)  # 减小字体和宽度
        self.home_nav_btn.pack(pady=(0, 8))
        
        # 添加分隔线
        tk.Frame(nav_frame, bg="#34495e", height=2).pack(fill=tk.X, pady=8)
        
        # 内置分类标题
        builtin_title = tk.Label(nav_frame, text="📦 内置分类", bg="#2c3e50", fg="#95a5a6",
                               font=("Segoe UI", 10, "bold"), anchor="w", padx=10)  # 减小字体
        builtin_title.pack(fill=tk.X, pady=(5, 3))
        
        # 内置分类按钮
        self.builtin_buttons_frame = tk.Frame(nav_frame, bg="#2c3e50")
        self.builtin_buttons_frame.pack(fill=tk.X, padx=5)
        ...
        for category in self.categories.keys():
            btn = tk.Button(self.builtin_buttons_frame, text=f"📄 {category}",
                           ...
                           bg="#34495e", fg="white", bd=0, padx=8, pady=4,  # 减少内边距
                           font=("Segoe UI", 9), anchor="w", width=18)  # 减小字体
        ...
        imported_title = tk.Label(nav_frame, text="📂 导入的文件", bg="#2c3e50", fg="#95a5a6", 
                                font=("Segoe UI", 10, "bold"), anchor="w", padx=10)  # 减小字体
        imported_title.pack(fill=tk.X, pady=(5, 3))
        
        # 导入文件容器
        self.imported_files_frame = tk.Frame(nav_frame, bg="#2c3e50")
        self.imported_files_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=(0, 8))
        ...
        tools_frame.pack(fill=tk.X, padx=5, pady=(0, 8))
        
        # 刷新按钮
        refresh_btn = tk.Button(tools_frame, text="🔄 刷新列表", command=self.refresh_file_list,
                              bg="#9b59b6", fg="white", bd=0, padx=8, pady=4,  # 减少内边距
                              font=("Segoe UI", 9))  # 减小字体
        refresh_btn.pack(fill=tk.X, pady=1)
    ...
    def create_menu(self):
        ...
        edit_menu.add_command(label="✏ 编辑内容", command=self.modify_content)
        ...
        tools_menu.add_command(label="▶️ 运行代码", command=self.run_python_code)  # 新增
    ...
    def create_home_page(self):
        ...
        # 使用Canvas和Frame实现可滚动的主页
        home_canvas = tk.Canvas(self.home_frame, bg="white", highlightthickness=0)
        home_scrollbar = tk.Scrollbar(self.home_frame, orient=tk.VERTICAL, command=home_canvas.yview)
        ...
        home_content = tk.Frame(home_canvas, bg="white")
        home_canvas.create_window((0, 0), window=home_content, anchor="nw")
        
        # 更新滚动区域
        home_content.bind("<Configure>", lambda e: home_canvas.configure(
            scrollregion=home_canvas.bbox("all")
        ))
        
        # 欢迎标题
        welcome_label = tk.Label(home_content, text="欢迎使用分类查看器",
                               bg="white", fg="#2c3e50", 
        ...
        desc_label = tk.Label(home_content, text=desc_text, 
                             bg="white", fg="#34495e", justify=tk.LEFT,
                             font=("Segoe UI", 12))  # 减小字体
        desc_label.pack(pady=10, padx=20)
        
        # 功能卡片
        card_frame = tk.Frame(home_content, bg="white")
        ...
        for card in cards:
            card_bg = tk.Frame(card_frame, bg=card["color"], bd=0, 
                              highlightthickness=0, width=200, height=100)  # 减小高度
            card_bg.grid(row=row, column=col, padx=5, pady=5, sticky="nsew")
            card_bg.grid_propagate(False)
            
            card_content = tk.Frame(card_bg, bg="white", padx=5, pady=5)  # 减少内边距
        ...
        # 最近文件标题
        recent_label = tk.Label(home_content, text="⏱ 最近文件", 
                              bg="white", fg="#2c3e50", 
                              font=("Segoe UI", 14, "bold"))  # 减小字体
        recent_label.pack(pady=(20, 5), anchor="w", padx=20)
        
        # 最近文件列表（示例）
        self.recent_files_frame = tk.Frame(home_content, bg="white")
        self.recent_files_frame.pack(fill=tk.X, padx=20, pady=(0, 20))
        
        # 状态栏
        self.status_var = tk.StringVar()
        self.status_var.set("就绪 | 欢迎使用分类查看器")
        status_bar = tk.Label(self.root, textvariable=self.status_var, bd=0, relief=tk.FLAT, 
                             anchor=tk.W, bg="#ecf0f1", fg="#666", font=("Segoe UI", 9))  # 减小字体
        status_bar.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=(0, 5))  # 减少内边距
    ...
    def create_text_viewer(self):
        ...
        self.text_paned = tk.PanedWindow(self.text_frame, orient=tk.VERTICAL, bg="#f0f2f5", sashwidth=4, sashrelief=tk.RAISED)  # 减小分隔条宽度
        ...
        # 固定行号区域
        self.line_number_frame = tk.Frame(code_frame, bg="#f5f5f5", width=40)  # 减小宽度
    ...
    def update_fixed_line_numbers(self, event=None):
        ...
        new_width = char_width * max_digits + 10  # 减少边距
        ...
        for line_num in range(first_visible_line, last_visible_line + 1):
            ...
            if line_num % 50 == 0 and line_num > 0:
                ...
                # 在虚线上方添加标记文本
                self.line_number_canvas.create_text(
                    3, y_pos + line_height / 2 - 8,  # 文本位置在虚线上方
                    text=f"第{line_num}行",
                    anchor=tk.W,
                    fill="#666",
                    font=("Segoe UI", 8)  # 减小字体
                )
    ...
    def create_pdf_viewer(self):
        ...
        pdf_container.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)  # 减少内边距
    ...
    def set_font(self):
        """设置高分辨率友好字体"""
        # 文本查看器字体
        self.viewer_font = font.Font(family="Consolas", size=12)  # 减小字体大小
        if hasattr(self, 'text_viewer'):
            self.text_viewer.configure(font=self.viewer_font)
        
        # 输出文本字体
        if hasattr(self, 'output_text'):
            self.output_text.configure(font=("Consolas", 10))  # 减小字体大小
        
        # 主页字体
        if hasattr(self, 'home_frame'):
            for widget in self.home_frame.winfo_children():
                if isinstance(widget, tk.Label):
                    widget.configure(font=("Segoe UI", 10))  # 减小字体大小
    ...
    def create_pdf_navigation(self):
        ...
        # 上一页按钮
        prev_btn = tk.Button(self.pdf_nav_frame, text="⬅️ 上页", 
                            command=self.prev_pdf_page,
                            bg="#3498db", fg="white", bd=0, padx=6, pady=3, font=("Segoe UI", 9))  # 减小按钮大小
        prev_btn.pack(side=tk.LEFT, padx=2)
        
        # 页码显示
        page_label = tk.Label(self.pdf_nav_frame, textvariable=self.pdf_page_var, 
                            bg="#f0f2f5", font=("Segoe UI", 9))  # 减小字体
        page_label.pack(side=tk.LEFT, padx=2)
        
        # 下一页按钮
        next_btn = tk.Button(self.pdf_nav_frame, text="➡️ 下页", 
        ...
        zoom_in_btn = tk.Button(self.pdf_nav_frame, text="➕ 放大", 
                              ...
                              bg="#2ecc71", fg="white", bd=0, padx=6, pady=3, font=("Segoe UI", 9))  # 减小按钮大小
        zoom_in_btn.pack(side=tk.LEFT, padx=2)
        
        # 缩小按钮
        zoom_out_btn = tk.Button(self.pdf_nav_frame, text="➖ 缩小", 
                               command=self.zoom_out,
                               bg="#e74c3c", fg="white", bd=0, padx=6, pady=3, font=("Segoe UI", 9))  # 减小按钮大小
    ...
    def modify_content(self):
        ...
        modify_dialog.geometry("1000x700")  # 减小对话框尺寸
        modify_dialog.transient(self.root)
        modify_dialog.grab_set()
        self.set_dialog_icon(modify_dialog)
        
        # 创建对话框框架
        dialog_frame = tk.Frame(modify_dialog, bg="white", padx=15, pady=15)  # 减少内边距
        ...
        save_btn = tk.Button(button_frame, text="💾 保存", 
                            ...
                            bg="#4CAF50", fg="white", bd=0, padx=15, pady=6, font=("Segoe UI", 10))  # 减小按钮
        save_btn.pack(side=tk.LEFT, padx=5)
        
        # 取消按钮
        cancel_btn = tk.Button(button_frame, text="❌ 取消", command=modify_dialog.destroy, 
                              bg="#F44336", fg="white", bd=0, padx=15, pady=6, font=("Segoe UI", 10))  # 减小按钮
    ...
    def open_calculator(self):
        ...
        calc_dialog.geometry("350x450")  # 减小对话框尺寸
        calc_dialog.transient(self.root)
        calc_dialog.grab_set()
        self.set_dialog_icon(calc_dialog)
        
        # 创建对话框框架
        dialog_frame = tk.Frame(calc_dialog, bg="white", padx=15, pady=15)  # 减少内边距
        dialog_frame.pack(fill=tk.BOTH, expand=True)
        ...
        calc_entry = tk.Entry(dialog_frame, textvariable=self.calc_var, 
                             font=("Consolas", 12), justify=tk.RIGHT,  # 减小字体
        ...
        result_label = tk.Label(dialog_frame, textvariable=self.calc_result, 
                              bg="white", fg="#333", font=("Consolas", 12),  # 减小字体
            ...
            for c, btn in enumerate(row):
                ...
                # 对于'='按钮，让它跨两列
                    ...
                    tk.Button(buttons_frame, text=btn, font=("Segoe UI", 10),  # 减小字体
                              ...
                              bg=bg_color, fg="black" if bg_color == "#E0E0E0" else "white",
                              bd=0, relief="ridge").grid(
                              row=r, column=2, columnspan=2, sticky="nsew", padx=1, pady=1)  # 减少间距
                else:
                    tk.Button(buttons_frame, text=btn, font=("Segoe UI", 10),  # 减小字体
                              command=lambda b=btn: self.on_calculator_button(b),
                              bg=bg_color, fg="black" if bg_color == "#E0E0E0" else "white",
                              bd=0, relief="ridge").grid(
                              row=r, column=c, sticky="nsew", padx=1, pady=1)  # 减少间距
    ...
    def extract_pdf_text(self):
        ...
        extract_dialog.geometry("800x600")  # 减小对话框尺寸
        extract_dialog.transient(self.root)
        extract_dialog.grab_set()
        self.set_dialog_icon(extract_dialog)
        
        # 创建对话框框架
        dialog_frame = tk.Frame(extract_dialog, bg="white", padx=15, pady=15)  # 减少内边距
        dialog_frame.pack(fill=tk.BOTH, expand=True)
    ...
    def manage_snippets(self):
        ...
        manage_dialog.geometry("700x500")  # 减小对话框尺寸
        manage_dialog.transient(self.root)
        manage_dialog.grab_set()
        self.set_dialog_icon(manage_dialog)
        
        # 创建对话框框架
        dialog_frame = tk.Frame(manage_dialog, bg="white", padx=15, pady=15)  # 减少内边距
        dialog_frame.pack(fill=tk.BOTH, expand=True)
        ...
        view_btn = tk.Button(button_frame, text="👁 查看片段", 
                            ...
                            bg="#3498db", fg="white", bd=0, padx=12, pady=5, font=("Segoe UI", 10))  # 减小按钮
        ...
        delete_btn = tk.Button(button_frame, text="🗑 删除片段", 
                              ...
                              bg="#F44336", fg="white", bd=0, padx=12, pady=5, font=("Segoe UI", 10))  # 减小按钮
        delete_btn.pack(side=tk.LEFT, padx=5)
        
        # 关闭按钮
        close_btn = tk.Button(button_frame, text="❌ 关闭", command=manage_dialog.destroy, 
                              bg="#9E9E9E", fg="white", bd=0, padx=12, pady=5, font=("Segoe UI", 10))  # 减小按钮
        close_btn.pack(side=tk.RIGHT, padx=5)
    ...
    def view_snippet(self, snippet_name):
        ...
        copy_btn = tk.Button(button_frame, text="📋 复制片段", 
                            ...
                            bg="#4CAF50", fg="white", bd=0, padx=12, pady=5, font=("Segoe UI", 10))  # 减小按钮
        copy_btn.pack(side=tk.LEFT, padx=5)
    ...
    def take_note(self):
        ...
        note_dialog.geometry("1200x800")  # 减小对话框尺寸
        note_dialog.transient(self.root)
        note_dialog.grab_set()
        self.set_dialog_icon(note_dialog)
        
        # 创建对话框框架
        dialog_frame = tk.Frame(note_dialog, bg="white", padx=15, pady=15)  # 减少内边距
        dialog_frame.pack(fill=tk.BOTH, expand=True)
        ...
        tk.Label(title_frame, text="标题:", bg="white", font=("Segoe UI", 10)).pack(side=tk.LEFT)  # 减小字体
        note_title = tk.Entry(title_frame, font=("Segoe UI", 10), width=40)  # 减小字体和宽度
        note_title.pack(side=tk.LEFT, padx=8, fill=tk.X, expand=True)
        note_title.focus()
    ...
    def manage_notes(self):
        ...
        edit_btn = tk.Button(button_frame, text="✏ 编辑笔记", 
                            ...
                            bg="#f39c12", fg="white", bd=0, padx=12, pady=5, font=("Segoe UI", 10))  # 减小按钮
    ...
    def edit_selected_note(self, notes_list):
        ...
        note_dialog.geometry("1000x700")  # 减小对话框尺寸
        note_dialog.transient(self.root)
        note_dialog.grab_set()
        self.set_dialog_icon(note_dialog)
